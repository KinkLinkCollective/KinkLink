...
public partial class WardrobeViewUi
{
    ...
    {
        foreach (var piece in controller.WardrobePieces)
        {
            var isSelected = controller.SelectedPieceId == piece.Id;

            if (isSelected)
            {
                ImGui.PushStyleColor(ImGuiCol.Header, KinkLinkStyle.PrimaryColor);
                if (ImGui.Selectable($"{piece.Name}##{piece.Id}", true))
                {
            ...
            {
                if (ImGui.Selectable($"{piece.Name}##{piece.Id}"))
                {
                    controller.SelectedPieceId = piece.Id;
                    controller.SelectedSetId = null;
                }
            ...
            {
                ImGui.SetTooltip($"Slot: {piece.Slot}");
            }
        }

        if (controller.WardrobePieces.Count == 0)
        {
    ...
    private void DrawSetsList()
    {
        ...
        if (controller.ImportedSets.Count == 0)
        {
