...
public class HonorificViewUi(HonorificViewUiController controller, PairsListComponentUi friendsList, CommandLockoutService commandLockoutService, SelectionManager selectionManager) : IDrawable
{
    ...
    public void Draw()
    {
        ImGui.BeginChild("HonorificContent", KinkLinkStyle.ContentSize, false, KinkLinkStyle.ContentFlags);

        var width = ImGui.GetWindowWidth();
        var padding = new Vector2(ImGui.GetStyle().WindowPadding.X, 0);

        var begin = ImGui.GetCursorPosY();
        SharedUserInterfaces.ContentBox("TitleSearch", KinkLinkStyle.PanelBackground, true, () =>
        {
        ...
        SharedUserInterfaces.ContentBox("HonorificSend", KinkLinkStyle.PanelBackground, false, () =>
        {
